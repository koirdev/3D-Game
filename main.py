...
PickedWeapon = 1 # None - Gun on table, 1 - Gun, 2 - Nothing
WeaponList = None
...
if PickedWeapon == 1:
	gun = Entity(model="assets/3d_models/gun.glb", scale=0.9,color=color.red,texture='white_cube',parent=camera.ui,rotation=(0,55,0),position=(0.5,-0.2,3),shader=lit_with_shadows_shader)
if PickedWeapon == 0:
	gunOnTable = Button(model="assets/3d_models/gun.glb",color=color.red,texture='white_cube',collider='box',parent=scene, scale=1.2, x=-1.5, y=6,z=0, shader=lit_with_shadows_shader)
...
if VIEW_MODE == 1:
	...
	text_entity = Text('ver. '+VERSION,scale=2,x=-00.1,y=0.5)
...
def update():
	...
	if PickedWeapon == 0:
		gunOnTable.rotation_y = gunOnTable.rotation_y + 1.2
	if VIEW_MODE == 1:
